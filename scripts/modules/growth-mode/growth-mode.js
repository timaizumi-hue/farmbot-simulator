(() => {
  ...
  const VERSION='25.15';
  const SAVE_KEY='farmbot_growth_session_v25_15';
  const OLD_KEYS=['farmbot_growth_session_v25_14','farmbot_growth_session_v25_13','farmbot_growth_session_v25_12'];
  const TOTAL_DAYS=90;
  ...
  function injectStyles(){
    if($('#growthModeV2515Style'))return;
    const st=document.createElement('style');st.id='growthModeV2515Style';
    st.textContent=`
      #growthHudV2515{position:fixed;left:50%;top:8px;transform:translateX(-50%);z-index:9800;width:min(1180px,calc(100vw - 18px));background:rgba(255,250,240,.98);border:2px solid #9bb68c;border-radius:14px;box-shadow:0 12px 36px rgba(0,0,0,.22);font-family:system-ui,-apple-system,'Segoe UI',sans-serif;color:#263027;display:grid;grid-template-columns:1fr auto;gap:9px;align-items:start;padding:8px 10px;box-sizing:border-box}.g-main{display:flex;align-items:center;gap:7px;flex-wrap:wrap;min-width:0}.g-badge{background:#25452a;color:#fff;border-radius:999px;padding:4px 9px;font-size:12px;white-space:nowrap}.g-strong{font-weight:800;font-size:16px;white-space:nowrap}.g-chip{background:#eef5e8;border:1px solid #d3e1c8;border-radius:999px;padding:4px 8px;font-size:12px;white-space:nowrap}.g-actions{display:flex;gap:5px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.g-actions button,#growthPanelV2515 button{border:1px solid #c8d2bd;background:#fff;border-radius:9px;min-height:30px;padding:4px 9px;font-weight:700;cursor:pointer}.g-actions button.active,#growthPanelV2515 button.active{background:#2f6b36!important;color:#fff!important;border-color:#2f6b36!important}.g-actions button.tool-active{background:#7a5b25!important;color:#fff!important;border-color:#7a5b25!important}.g-hidden{display:none!important}.g-plant-strip{display:flex;gap:5px;align-items:center;overflow:auto;padding-top:3px;width:100%}.g-mini-plant{min-width:108px;border:1px solid #ddd2bf;background:#fff;border-radius:9px;padding:4px 6px;font-size:11px}.g-mini-plant b{display:block;font-size:12px}.g-mini-plant .minirow{display:flex;justify-content:space-between;gap:4px}.g-mini-plant .bar{height:4px;background:#e8e1d2;border-radius:9px;overflow:hidden;margin-top:3px}.g-mini-plant .bar i{display:block;height:100%;background:#5e9d4f}.g-mini-plant.warn .bar i{background:#d5a63b}.g-mini-plant.bad .bar i{background:#c75b4e}.g-save-indicator{font-size:11px;color:#60705f}
      #growthPanelV2515{position:fixed;inset:68px 12px 12px 12px;z-index:9790;background:#f7f4ec;border:2px solid #d7ceb9;border-radius:16px;box-shadow:0 22px 60px rgba(0,0,0,.30);display:grid;grid-template-rows:auto 1fr;overflow:hidden;color:#263027;font-family:system-ui,-apple-system,'Segoe UI',sans-serif}.gp-head{display:flex;justify-content:space-between;gap:12px;align-items:center;background:#fffaf0;border-bottom:1px solid #ddd2bf;padding:10px 14px}.gp-head h2{margin:0;font-size:20px}.gp-head p{margin:2px 0 0;color:#60705f;font-size:12px}.gp-body{display:grid;grid-template-columns:250px 1fr 310px;gap:10px;padding:10px;min-height:0}.gp-card{background:#fff;border:1px solid #dfd7c9;border-radius:12px;padding:10px;overflow:auto;min-height:0}.gp-card h3{margin:2px 0 10px;font-size:15px}.gp-stat{display:flex;justify-content:space-between;gap:8px;border-bottom:1px dashed #e4ddd1;padding:7px 0}.gp-stat span{color:#667064}.gp-stat strong{text-align:right}.forecast{display:grid;gap:6px}.forecast div{display:grid;grid-template-columns:44px 52px 1fr;gap:5px;align-items:center;border:1px solid #eee6d8;background:#fbfaf5;border-radius:9px;padding:6px;font-size:12px}.plant-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px}.plant-card{border:1px solid #e4ddd1;border-radius:12px;background:#fbfaf5;padding:9px}.plant-head{display:flex;justify-content:space-between;gap:8px}.plant-head strong{font-size:15px}.plant-head span{font-size:12px;border-radius:999px;padding:3px 8px}.good{background:#dff3df;color:#276b32}.warn{background:#fff0bd;color:#745600}.bad{background:#ffd7d0;color:#8b2c20}.bars{display:grid;gap:6px;margin-top:8px}.bars label{display:grid;grid-template-columns:42px 1fr 42px;gap:8px;align-items:center;font-size:12px}.bars meter{width:100%;height:12px}.mini-map{background:#e6dbc4;border:1px solid #a99475;border-radius:10px;overflow:hidden;margin-bottom:8px}.mini-map svg{display:block;width:100%;height:150px}.mini-map rect{fill:#e3d2b6;stroke:#9b8c70}.mini-map circle{fill:#5d9d51;stroke:#2e5e29;stroke-width:2}.mini-map text{font-size:12px;fill:#2f3e31}.lock-note{font-size:12px;background:#f1eadc;border:1px solid #e1d7c5;border-radius:9px;padding:7px;margin:8px 0}.g-log{display:grid;gap:6px;margin-top:10px}.g-log div{border:1px solid #eee6d8;border-radius:9px;background:#fbfaf5;padding:7px;font-size:12px}.muted{color:#647164;font-size:12px}.tool-help{font-size:12px;background:#fff8df;border:1px solid #eadca8;border-radius:9px;padding:7px;margin-top:6px}.gp-footer-actions{display:flex;gap:6px;flex-wrap:wrap;margin-top:10px}
  ...
  function ensureHud(){
    ...
    h=document.createElement('div');h.id='growthHudV2515';h.innerHTML=`<div><div class="g-main"><span class="g-badge" id="gSeason">育成</span><span class="g-strong" id="gDate">--</span><span class="g-chip" id="gDay">--</span><span class="g-chip" id="gStage">--</span><span class="g-chip" id="gSpeed">停止</span><span class="g-chip" id="gWeather">--</span><span class="g-chip" id="gTool">ツール: なし</span><span class="g-save-indicator" id="gSaveState">自動保存</span></div><div class="g-plant-strip" id="gPlantStrip"></div></div><div class="g-actions"><button id="gPause">停止</button><button id="gPlay">進む</button><button id="gFast">早送り</button><button id="gStep">1日</button><button data-gtool="fertilizer">肥料</button><button data-gtool="pesticide">殺虫剤</button><button data-gtool="weed">雑草駆除</button><button id="gMusic">音楽</button><button id="gDetail">詳細</button><button id="gSave">保存</button></div>`;
    document.body.appendChild(h);$('#gPause',h).onclick=pause;$('#gPlay',h).onclick=()=>play(1);$('#gFast',h).onclick=()=>play(8);$('#gStep',h).onclick=stepDay;$('#gDetail',h).onclick=()=>{ensurePanel().classList.remove('g-hidden');render();};$('#gSave',h).onclick=()=>{log('手動保存しました。');autoSave();render();};$('#gMusic',h).onclick=toggleMusic;h.querySelectorAll('[data-gtool]').forEach(b=>b.onclick=()=>setTool(session?.activeTool===b.dataset.gtool?'none':b.dataset.gtool));return h;
  }
  function ensurePanel(){
    ...
    document.body.appendChild(p);$('#gpMin',p).onclick=()=>p.classList.add('g-hidden');$('#gpPause',p).onclick=pause;$('#gpPlay',p).onclick=()=>play(1);$('#gpFast',p).onclick=()=>play(8);$('#gpStep',p).onclick=stepDay;p.querySelectorAll('[data-gtool]').forEach(b=>b.onclick=()=>setTool(session?.activeTool===b.dataset.gtool?'none':b.dataset.gtool));return p;
  }
