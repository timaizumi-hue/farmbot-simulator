(() => {
  ...
  function speedLabel(speed){
    ...
    if(speed===1)return '進む ×1（現実時間）';
    if(speed===FAST_SPEEDS.slow)return '早送り ＞（1秒=1分）';
    if(speed===FAST_SPEEDS.mid)return '早送り ＞＞（1秒=6分）';
    if(speed===FAST_SPEEDS.high)return '早送り ＞＞＞（90日≈1時間）';
    return `速度 x${speed}`;
  }
  ...
  function getSeasonPlants(kind, amount='medium'){
    ...
    return arr.map((a,i)=>({id:'gplant_'+i,type:a[0],name:cropName(a[0])+(i+1),x:a[1],y:a[2],stage:a[3],waterPct:28+((i*5)%12),growth:a[3]==='seedling'?14:32,health:86,fertility:58+((i*9)%20),bugs:false,alive:true,yieldScore:0}));
  }
  ...
  function normalize(s){if(!s||!Array.isArray(s.plants))return null;const meta=SEASONS[s.kind]||SEASONS.spring_growth;s.version=VERSION;s.label=s.label||meta.label;s.startMonth=s.startMonth||meta.startMonth;s.base=s.base||meta.base;s.day=clamp(Number(s.day||0),0,TOTAL_DAYS);s.running=!!s.running;s.speed=Number(s.speed||0);if(s.speed===8)s.speed=FAST_SPEEDS.high;s.lockedPlants=true;s.activeTool=s.activeTool||'none';s.difficulty=s.difficulty||'normal';s.difficultyLabel=(DIFFICULTY[s.difficulty]||DIFFICULTY.normal).label;s.plantAmount=s.plantAmount||'medium';s.notes=Array.isArray(s.notes)?s.notes:[];s.weeds=Array.isArray(s.weeds)?s.weeds:[];s.plants=s.plants.map(normalizePlant);return s;}
  function makeSession(kind, options={}){
    ...
    session.plants=mainPlants.map((p,i)=>normalizePlant({...p,stage:'seedling',growth:6+((i*5)%10),waterPct:28+((i*5)%10),health:88,fertility:58+((i*9)%18)},i));
  ...
  function ensureClock(){
    let c=$('#growthClockV2517');if(c){makeClockDraggable(c);return c;}
    c=document.createElement('div');c.id='growthClockV2517';c.className='g-clock g-hidden';
    c.innerHTML='<div class="clock-face"><span class="clock-hand clock-hour"></span><span class="clock-hand clock-min"></span></div><div class="clock-label" id="clockTime">00:00</div><div class="clock-sub" id="clockDate">--</div><div class="clock-sub" id="clockSpeed">停止 / ×1は現実時間</div><div class="clock-progress"><i id="clockProgress"></i></div>';
    document.body.appendChild(c);makeClockDraggable(c);return c;
  }
  function simTime(){
  ...
  function renderClock(){
    ...
    $('#clockSpeed',c).textContent=session.running?`${speedLabel(session.speed)} で進行中`:'停止中';
    const pr=$('#clockProgress',c); if(pr)pr.style.width=Math.round(t.frac*100)+'%';
  }
  function makeClockDraggable(c){
    if(!c||c.dataset.dragReady)return;
    c.dataset.dragReady='1';
    try{const saved=JSON.parse(localStorage.getItem('farmbot_growth_clock_pos_v1')||'null');if(saved&&Number.isFinite(saved.left)&&Number.isFinite(saved.top)){c.style.left=saved.left+'px';c.style.top=saved.top+'px';c.style.right='auto';}}catch{}
    let drag=null;
    c.addEventListener('pointerdown',e=>{drag={x:e.clientX,y:e.clientY,left:c.getBoundingClientRect().left,top:c.getBoundingClientRect().top};c.setPointerCapture?.(e.pointerId);});
    c.addEventListener('pointermove',e=>{if(!drag)return;const left=clamp(drag.left+e.clientX-drag.x,4,window.innerWidth-c.offsetWidth-4);const top=clamp(drag.top+e.clientY-drag.y,4,window.innerHeight-c.offsetHeight-4);c.style.left=left+'px';c.style.top=top+'px';c.style.right='auto';});
    c.addEventListener('pointerup',()=>{if(!drag)return;const r=c.getBoundingClientRect();localStorage.setItem('farmbot_growth_clock_pos_v1',JSON.stringify({left:r.left,top:r.top}));drag=null;});
  }
  ...
  function updateButtons(){['gPause','gpPause','gPlay','gpPlay','gFast1','gFast2','gFast3','gpFast1','gpFast2','gpFast3'].forEach(id=>$('#'+id)?.classList.remove('active'));if(!session?.running){$('#gPause')?.classList.add('active');$('#gpPause')?.classList.add('active');}else if(session.speed===1){$('#gPlay')?.classList.add('active');$('#gpPlay')?.classList.add('active');}else if(session.speed===FAST_SPEEDS.slow){$('#gFast1')?.classList.add('active');$('#gpFast1')?.classList.add('active');}else if(session.speed===FAST_SPEEDS.mid){$('#gFast2')?.classList.add('active');$('#gpFast2')?.classList.add('active');}else if(session.speed===FAST_SPEEDS.high){$('#gFast3')?.classList.add('active');$('#gpFast3')?.classList.add('active');}document.querySelectorAll('[data-gtool]').forEach(b=>b.classList.toggle('tool-active',session?.activeTool===b.dataset.gtool));$('#gToolBtn')?.classList.toggle('tool-active',!!session?.activeTool&&session.activeTool!=='none');}
  ...
  function advanceDay(){
    if(!session)return;
    // 1日進むたびに、練習画面側の水分と水跡を一緒に乾かしてからHUDへ同期する。
    window.FarmBotAppBridge?.dryDownGrowthWater?.(1);
    syncMoistureFromMain();
    const d=Math.floor(session.day);const w=weatherAt(d);
  ...
  function play(speed=1){if(!session)return;session.running=true;session.speed=speed;if(timer)clearInterval(timer);lastTick=Date.now();timer=setInterval(()=>{if(!session?.running)return;const now=Date.now();const delta=now-lastTick;lastTick=now;const before=Math.floor(session.day);session.day=clamp(session.day+delta/REAL_DAY_MS*session.speed,0,TOTAL_DAYS);const after=Math.floor(session.day);if(after>before){for(let i=before;i<after;i++)advanceDay();}render();},500);render();}
  ...
  function open(kind, options={}){injectStyles();showClock=localStorage.getItem('farmbot_growth_clock_v1')==='1';if(kind==='load'){if(!load())makeSession('spring_growth',options);}else{try{localStorage.removeItem(SAVE_KEY);}catch{} makeSession(kind||'spring_growth',options);}const resetMain=!!session?._freshStart;applyToMain(resetMain);if(resetMain)window.FarmBotAppBridge?.resetGrowthWaterSurface?.();syncMoistureFromMain();ensureHud();ensurePanel().classList.add('g-hidden');ensureAutoSave();render();}
