(() => {
  ...
  const TOTAL_DAYS=90;
  const DAY_MS=24*60*60*1000; // 通常再生は現実の時間と同じ：1日=24時間
  const SPEED_STEPS=[1,60,1440];
  ...
  function normalize(s){if(!s||!Array.isArray(s.plants))return null;const meta=SEASONS[s.kind]||SEASONS.spring_growth;s.version=VERSION;s.label=s.label||meta.label;s.startMonth=s.startMonth||meta.startMonth;s.base=s.base||meta.base;s.day=clamp(Number(s.day||0),0,TOTAL_DAYS);s.running=!!s.running;s.speed=[1,60,1440].includes(Number(s.speed||0))?Number(s.speed||0):0;s.lockedPlants=true;s.activeTool=s.activeTool||'none';s.difficulty=s.difficulty||'normal';s.difficultyLabel=(DIFFICULTY[s.difficulty]||DIFFICULTY.normal).label;s.plantAmount=s.plantAmount||'medium';s.notes=Array.isArray(s.notes)?s.notes:[];s.weeds=Array.isArray(s.weeds)?s.weeds:[];s.plants=s.plants.map(normalizePlant);return s;}
  ...
  function makeClockDraggable(c){
    if(!c||c.dataset.dragReady)return;c.dataset.dragReady='1';
    ...
    let drag=null;
    c.addEventListener('pointerdown',ev=>{if(ev.target.closest('button,select,input'))return;const r=c.getBoundingClientRect();drag={dx:ev.clientX-r.left,dy:ev.clientY-r.top};c.setPointerCapture?.(ev.pointerId);ev.preventDefault();});
    c.addEventListener('pointermove',ev=>{if(!drag)return;const left=clamp(ev.clientX-drag.dx,4,window.innerWidth-80);const top=clamp(ev.clientY-drag.dy,4,window.innerHeight-80);c.style.left=left+'px';c.style.top=top+'px';c.style.right='auto';});
  ...
  function ensureClock(){
    ...
    c.innerHTML='<div class="clock-face"><span class="clock-hand clock-hour"></span><span class="clock-hand clock-min"></span></div><div class="clock-label" id="clockTime">00:00</div><div class="clock-sub" id="clockDate">--</div><div class="clock-sub" id="clockSpeed">停止 / 通常は現実時間</div><div class="clock-sub">ドラッグで移動</div><div class="clock-progress"><i id="clockProgress"></i></div>';
  ...
  function renderClock(){
    ...
    $('#clockSpeed',c).textContent=session.running?`速度 ${speedLabel(session.speed)} で進行中`:'停止中';
  ...
  function speedLabel(speed){if(speed>=1440)return'>>>';if(speed>=60)return'>>';if(speed>=1)return'>';return'停止';}
  function updateButtons(){['gPause','gpPause','gPlay','gpPlay','gFast','gpFast','gFast2','gpFast2'].forEach(id=>$('#'+id)?.classList.remove('active'));if(!session?.running){$('#gPause')?.classList.add('active');$('#gpPause')?.classList.add('active');}else if(session.speed>=1440){$('#gFast2')?.classList.add('active');$('#gpFast2')?.classList.add('active');}else if(session.speed>=60){$('#gFast')?.classList.add('active');$('#gpFast')?.classList.add('active');}else{$('#gPlay')?.classList.add('active');$('#gpPlay')?.classList.add('active');}document.querySelectorAll('[data-gtool]').forEach(b=>b.classList.toggle('tool-active',session?.activeTool===b.dataset.gtool));$('#gToolBtn')?.classList.toggle('tool-active',!!session?.activeTool&&session.activeTool!=='none');}
  ...
  function play(speed=1){if(!session)return;speed=SPEED_STEPS.includes(Number(speed))?Number(speed):1;session.running=true;session.speed=speed;if(timer)clearInterval(timer);lastTick=Date.now();timer=setInterval(()=>{if(!session?.running)return;const now=Date.now();const delta=now-lastTick;lastTick=now;const before=Math.floor(session.day);session.day=clamp(session.day+delta/DAY_MS*session.speed,0,TOTAL_DAYS);const after=Math.floor(session.day);if(after>before){for(let i=before;i<after;i++)advanceDay();}render();},1000);render();}
